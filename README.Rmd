...
Two types of models will be fitted, tested, and analyzed for accuracy and misclassification of this prediction. As mentioned previously, the first will be an Ensemble Model via Random Forests and the second will be a Linear Regression model via a Generalized Linear Model. Both test all possible predictor variables.  

The fit process for both will utilize the `caret` package and available relevant options for each model type, with more detail to follow. For each fit, we will rely on the functionality of the `caret` package to assist with k-fold cross validation, centering, and scaling.  
